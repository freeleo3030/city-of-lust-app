  ...
  useEffect(() => {
    supabase.from('female_characters').select('*').then(({ data, error }) => {
      ...
      const dbChars: FemaleCharacterData[] = data.map((row: any) => ({
        ...
        erogenous: row.stats?.erogenous ?? null,
        prefAge: row.stats?.prefAge ?? { age20:34, age30:33, age40:33 },
        prefLook: row.stats?.prefLook ?? { face:25, height:25, body:25, fashion:25 },
        prefWealth: row.stats?.prefWealth ?? 50,
        smSelf: row.stats?.smTendency ?? 0,
        smPreferMale: row.sm_prefer_male ?? 0,
        interests: row.stats?.interestTags ?? [],
        interestTags: row.stats?.interestTags ?? [],
        dislikes: row.stats?.dislikeTags ?? [],
        ...
        personality: row.stats?.personality ?? { introvert: 3, indirect: 3, friendly: 3 },
        interestCustom: row.stats?.interestCustom ?? '',
        dislikeTags: row.stats?.dislikeTags ?? [],
        dislikeCustom: row.stats?.dislikeCustom ?? '',
        appearanceDesc: row.stats?.appearanceDesc ?? '',
        hairColor: row.stats?.hairColor ?? '',
        hairLength: row.stats?.hairLength ?? '',
        glasses: row.stats?.glasses ?? false,
        heightCm: row.height_cm ?? 160,
